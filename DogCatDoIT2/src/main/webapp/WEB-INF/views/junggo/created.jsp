...
function sendOk() {
    var j = document.faqForm;

    if(! j.categoryNum.value) {
        alert("카테고리를 선택하세요. ");
        j.categoryNum.focus();
        return;
    }
    
	var str = j.subject.value;
    if(!str) {
        alert("제목을 입력하세요. ");
        j.subject.focus();
        return;
    ...
    j.action="${pageContext.request.contextPath}/junggo/${mode}";
    j.submit();
}
</script>
...
$(function(){
	...
	$("body").on("click", ".btnCategoryUpdate", function(){
		...
		$("#category-dialog").dialog({
			  ...
			  title: '카테고리 수정',
			  open : function(){
				var url="${pageContext.request.contextPath}/junggo/listAllCategory?tmp="+(new Date()).getTime();
				$(".category-list").load(url);  
			  },
			  close: function(event, ui) {
				  $("form[name=junggoForm] select[name=categoryNum]").find('option').remove();

				  var url="${pageContext.request.contextPath}/junggo/listCategory";
				  ...
				  var fn=function(data) {
						...
						var mode="${mode}";
						if(mode=="update") {
							$("form[name=junggoForm] select[name=categoryNum]").val("${dto.categoryNum}");
						}
				  };
	...
	$("body").on("click", ".btnCategorySend", function(){	
		...
		var url="${pageContext.request.contextPath}/junggo/insertCategory";
		...
		var fn = function(data){
			...
			url="${pageContext.request.contextPath}/junggo/listAllCategory?tmp="+(new Date()).getTime();
			$(".category-list").load(url); 
		};
	...
	$("body").on("click", ".btnUpdateOkIcon", function(){	
		...
		category = encodeURIComponent(category);
		var url="${pageContext.request.contextPath}/junggo/updateCategory";
	...
	$("body").on("click", ".btnDeleteIcon", function(){	
		...
		var url="${pageContext.request.contextPath}/junggo/deleteCategory";
...
<div class="container body-container">
	<div class="body-title">
		<h2><i class="icofont-junggo"></i> 중고거래 게시판 </h2>
	</div>
