...
<link rel="stylesheet"
	...
	type="text/css">
<script type="text/javascript"
	src="${pageContext.request.contextPath}/resources/jquery/js/jquery-ui.min.js"></script>
...
<script type="text/javascript">
				...
				.change(
						function() {
							...
							if ($("input[name=deliveryInfo]:checked").val() == "basic") { // 기본배송지(회원정보)
								...
								$("#deliveryForm .searchAddrBtn").css(
										"display", "none");

								// 인풋 요소들 readonly:true
								$("#deliveryForm input").not(
										"input[name=orderMemo]").prop(
										"readonly", true);
							} else { // 신규 배송지
								...
								$("#deliveryForm input").each(function() {
									$(this).not("input:radio").val("");
								});

								$("#deliveryForm input")
										.not(
												"input[name=orderMemo], input[name=diZip], input[name=diAddr1]")
										.prop("readonly", false);
							}
						});
	});
	...
	function calcTotalDiscount() {
		...
		td = parseInt(totalItemDiscount) + parseInt(memberDiscount)
				+ parseInt(couponDiscount) + parseInt(pointDiscount);
	...
	$(function() {
		...
		$("#searchCoupon")
				.on(
						"click",
						function() {
							let couponName = $(this).closest("form").find(
									"input").val();
		...
		function showResult(data) {
			...
			$(".searchResultLayout .deadline")
					.text("~ " + data.coupon.deadline);
		}
	...
	$(function() {
		...
		$(".btnConfirm").click(
				function() {
					...
					if (!f.diTel1.value.trim()
							|| !(/^[0-9]{3}$/).test(f.diTel1.value)) {
						f.diTel1.focus();
						alert("휴대폰 번호를 입력해주세요.");
						return false;
					}

					if (!f.diTel2.value.trim()
							|| !(/^[0-9]{4}$/).test(f.diTel2.value)) {
						f.diTel2.focus();
						alert("휴대폰 번호를 입력해주세요.");
						return false;
					}

					if (!f.diTel3.value.trim()
							|| !(/^[0-9]{4}$/).test(f.diTel3.value)) {
						f.diTel3.focus();
						alert("휴대폰 번호를 입력해주세요.");
						return false;
					}
...
<div class="container">
	<form id="orderForm" name="orderForm" method="post">
		...
		<table class="table1 itemList">
			...
			<tbody>
				<c:forEach items="${itemList}" var="item" varStatus="st">
					<tr>
						<td><a
							href="${pageContext.request.contextPath}/shopping/article?num=${item.itemId}">
								<img class="product-img"
								src="${pageContext.request.contextPath}/uploads/item/${item.saveFilename}"
								alt="사진">
						</a></td>
						<td>${item.itemName}<br> [옵션 :
							${item.optionName}&nbsp;${item.detailName}]
						</td>
						<td><fmt:formatNumber type="number" value="${item.itemSalePrice}"
								maxFractionDigits="3" />원</td>
						<td>${item.count}개</td>
						<td>기본배송</td>
						<td class="itemTotalAmount"
							data-itemTotalAmount="${item.itemSalePrice * item.count}"
							data-itemTotalDiscount="${item.discountPrice}"><fmt:formatNumber
								type="number" value="${item.itemSalePrice * item.count}"
								maxFractionDigits="3" />원 <c:if test="${from=='cart'}">
								<input type="hidden" name="itemList[${st.index}].cartIdx"
									value="${item.cartIdx}">
							</c:if> <input type="hidden" name="itemList[${st.index}].totalPrice"
							value="${item.itemSalePrice * item.count}"> <input type="hidden"
							name="itemList[${st.index}].detailId" value="${item.detailId}"> <input
							type="hidden" name="itemList[${st.index}].count" value="${item.count}">
						</td>
					</tr>
				</c:forEach>
			</tbody>
			<tfoot>
				<tr class="tr-bottom">
					<td colspan="6" align="right"
						style="text-align: right; padding-right: 15px;">상품금액합계: <span
						class="resultSpan"><span class="totalItemPrice"></span>원</span>
					</td>
				</tr>
			</tfoot>
		</table>
		...
		<table class="table2">
			<tr class="tr-top">
				<td>주문하시는분</td>
				<td><input type="text" name="orName" id="orName" value="${mdto.name}"
					class="boxTF orName" readonly="readonly"></td>
			</tr>
			<tr>
				<td>주&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;소</td>
				<td><input type="text" name="orAddr" id="orAddr" value="${mdto.addr}"
					class="boxTF" readonly="readonly"></td>
			</tr>
			<tr>
				<td>핸드폰번호</td>
				<td><input type="text" name="orTel1" id="orTel1" value="${mdto.tel1}"
					class="boxTF" readonly="readonly"> - <input type="text"
					name="orTel2" id="orTel2" value="${mdto.tel2}" class="boxTF"
					readonly="readonly"> - <input type="text" name="orTel3" id="orTel3"
					value="${mdto.tel3}" class="boxTF" readonly="readonly"></td>
			</tr>
			<tr class="tr-bottom">
				<td>이&nbsp;&nbsp;&nbsp;메&nbsp;&nbsp;&nbsp;일</td>
				<td><input type="text" name="orEmail" id="orEmail" class="boxTF"
					value="${mdto.email}" readonly="readonly"></td>
			</tr>
		</table>
		...
		<div id="deliveryForm">
			<table class="table2">
				<tr class="tr-top">
					<td>배송지선택</td>
					<td><input type="radio" id="basicDI" name="deliveryInfo" value="basic">
						<label for="basicDI">기본배송지(주문자정보와동일)</label> <input type="radio"
						id="newDI" name="deliveryInfo" value="new"> <label for="newDI">신규배송지</label>
					</td>
				</tr>
				<tr>
					<td>받으실분</td>
					<td><input type="text" name="diName" id="diName" class="boxTF">
					</td>
				</tr>
				<tr>
					<td rowspan="3">받으실곳</td>
					<td><input type="text" name="diZip" id="diZip" class="boxTF"
						readonly="readonly">
						<button type="button" class="btn searchAddrBtn" onclick="daumPostcode()">우편번호검색</button>
					</td>
				</tr>
				<tr>
					<td class="td2"><input type="text" name="diAddr1" id="diAddr1"
						class="boxTF" readonly="readonly"></td>
				</tr>
				<tr>
					<td class="td2"><input type="text" name="diAddr2" id="diAddr2"
						class="boxTF"></td>
				</tr>
				<tr>
					<td>핸드폰번호</td>
					<td><input type="text" name="diTel1" id="diTel1" class="boxTF"
						maxlength="3"> - <input type="text" name="diTel2" id="diTel2"
						class="boxTF" maxlength="4"> - <input type="text" name="diTel3"
						id="diTel3" class="boxTF" maxlength="4"></td>
				</tr>
				<tr class="tr-bottom">
					<td>남기실 말씀</td>
					<td><input type="text" name="orderMemo" id="orderMemo" class="boxTF">
					</td>
				</tr>
			</table>
		</div>
		...
		<table class="table2 payAmountTable">
			<tr class="tr-top">
				<td>상품합계금액</td>
				<td><span class="totalItemPrice"></span> 원 <input type="hidden"
					name="totalItemPrice"></td>
			</tr>
			<tr>
				<td>배&nbsp;&nbsp;송&nbsp;&nbsp;비</td>
				<td><fmt:formatNumber type="number" value="2500" maxFractionDigits="3" />
					원 <input type="hidden" name="deliveryPrice" value="2500"></td>
			</tr>
			<tr>
				<td>즉시할인액</td>
				<td><span class="totalItemDiscount"></span> 원 <input type="hidden"
					name="totalItemDiscount"></td>
			</tr>
			<tr>
				<td>회원할인</td>
				<td><span class="memberDiscount">0</span> 원 [ 등급:
					${slevelInfo.levelName} | 할인율 : ${slevelInfo.rate} % ] <input type="hidden"
					name="memberDiscount" value="0"></td>
			</tr>
			<tr>
				<td>쿠폰할인액</td>
				<td><input type="text" name="couponDiscount" id="couponDiscount"
					class="boxTF tright" value="0" readonly="readonly"> 원
					<button type="button" class="btn couponBtn">쿠폰적용및조회</button> <span
					class="couponInfo"></span> <input type="hidden" name="couponName">
				</td>
			</tr>
			<tr>
				<td>포인트적용</td>
				<td><input type="number" name="pointDiscount" id="pointDiscount"
					class="boxTF tright" value="0"> 원 (보유포인트: <fmt:formatNumber
						type="number" value="${mdto.point}" maxFractionDigits="3" /> 원)</td>
			</tr>
			<tr>
				<td>총 할인금액</td>
				<td><span class="totalDiscount"></span> 원 <input type="hidden"
					name="totalDiscount"></td>
			</tr>
			<tr class="tr-bottom">
				<td>총 결제금액</td>
				<td><span class="totalPayment"></span> 원 <input type="hidden"
					name="totalPayment"></td>
			</tr>
		</table>
	...
	<div class="modal fade" id="couponModal" tabindex="-1"
		...
		<div class="modal-dialog">
			<div class="modal-content">
						...
						<div class="form-row">
							<div class="col-8">
								<input type="text" class="form-control" name="couponName"
									placeholder="사용하실 쿠폰의 이름을 입력하세요.">
							</div>
							<div class="col">
						...
						<table class="table table-striped">
							...
							<tbody>
								<tr class="searchResult display-none">
									...
									<th scope="row" class="rate"></th>
									<td class="deadline"></td>
								</tr>
								<tr class="result-none display-none">
									<td colspan="3" align="center">사용할 수 없는 쿠폰입니다.</td>
								<tr>
							</tbody>
						</table>
					</div>
				...
				<div align="center">
					<p>
						<strong>쿠폰은 1장만 사용 가능합니다.</strong>
					</p>
				</div>
