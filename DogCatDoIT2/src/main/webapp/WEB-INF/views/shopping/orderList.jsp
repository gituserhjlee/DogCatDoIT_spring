...
function printOrderInfo(data) {
	...
	for(let idx=0; idx< data.order.itemList.length; idx++) {
		...
		out += "</td>";
		out += "<td>" + itemName + "<br>[ 옵션 : " + optionName + " " + detailName +" ]</td>";
		if(idx == 0)
			out += "<td style='width: 14%;' class='cell-center' rowspan='" + delivery_rowspan + "'>기본배송<br>"+ toLocaleString(deliveryPrice) +" 원</td>";
		out += "<td style='width: 14%;' class='cell-center'>" + toLocaleString(itemSalePrice) + " 원</td>";
		out += "<td style='width: 14%;' class='cell-center'>" + count + "</td>";
		out += "<td style='width: 14%;' class='cell-center'>" + toLocaleString(totalPrice) + " 원</td>";
		out += "</tr>";
	}
...
function setControlBtns(state) {
	...
	$(".orderCancelBtn, .exchangeApplyBtn, .refundApplyBtn").addClass("display-none");
}
